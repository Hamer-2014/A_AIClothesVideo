...
if (!jobId) {
  ...
  const response = await fetch(`${appUrl}/api/virtual-try-on`, { method: "POST", headers: { "content-type": "application/json", cookie, "idempotency-key": `virtual-try-on-smoke:${randomUUID()}` }, body: JSON.stringify({ mode: "front_only", skuName: "staging-smoke", sourceAssetIds: { front: frontAssetId } }) });
...
while (Date.now() < deadline) {
  ...
  try {
    const { rows } = await pool.query("select status from virtual_tryon_jobs where id = $1", [jobId]);
...
const pool = new pg.Pool({ connectionString: process.env.DATABASE_URL });
try {
  const { rows } = await pool.query("select j.status, j.reserved_ledger_id, j.captured_ledger_id, p.id as pack_id, a.r2_key, f.verdict, (select count(*)::int from garment_fidelity_results x where x.appearance_pack_id = p.id and x.scope = 'cross_view') as cross_count from virtual_tryon_jobs j join appearance_packs p on p.virtual_tryon_job_id = j.id join appearance_pack_assets a on a.appearance_pack_id = p.id and a.view = 'front' left join garment_fidelity_results f on f.appearance_pack_id = p.id and f.scope = 'view' and f.view = 'front' where j.id = $1 order by p.version desc limit 1", [jobId]);
  const row = rows[0];
  if (!row?.reserved_ledger_id || !row?.captured_ledger_id || !row?.r2_key || row.verdict !== "pass" || row.cross_count !== 0) throw new Error("virtual_tryon_smoke_delivery_verification_failed");
  ...
  console.log(`OK: virtual try-on smoke job=${jobId} status=${row.status}`);
