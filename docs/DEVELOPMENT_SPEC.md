...
- 免费试用默认低分辨率、带水印、无音频；付费默认高分辨率、无水印、包含音频，用户侧不暴露供应商具体分辨率。
...
- [ ] 下载 R2 segment 视频。
- [ ] ffmpeg 拼接/转码。
- [ ] 上传前校验任务画幅、生成档位最低短边，并检测 Standard/Strict 多段成片至少 1 秒的冻结。
- [ ] 生成封面。
- [ ] 根据 `post_qa_mode` 抽帧。
- [ ] 上传最终视频、封面、抽帧图到 R2。
- [ ] 清理临时文件。
...
- 8 秒使用普通 QA 抽帧计划：standard 固定 5 帧，strict 固定 6 帧；16/24/32 秒保留这些均匀帧并增加每个 8 秒拼接边界帧。
