...
3 组双语文章应同批发布，确保每篇都能获得父页面回链并拥有至少 2 个同层相关页面。代码已完成以下接入，部署时仍需逐项验收：

1. `/guides` 与 `/zh/guides` 索引入口。
2. 8 个双语索引/文章 URL 的 sitemap、canonical、title、description 和 hreflang。
3. 英文与中文三图专题页的 3 篇文章回链。
4. 页头、移动导航和页脚的指南入口。
...
- CTA target: `/workspace?mode=trial&preset=minimal_studio`；中文 `/zh/workspace?mode=trial&preset=minimal_studio`
- Unique value: 按“一个 SKU 要回答几个购买问题”选择时长，并结合真实的 1/2/3 个 8 秒镜头结构说明素材要求，而不是把更长的视频默认说成更好。
- Reader stage: Consideration / Decision
- Required examples or assets: 同一 SKU 的 8、16、24 秒分镜时间轴；对应使用场景示意
- Status: Ready，双语页面已实现，须同批部署

English SEO Title：`8, 16, or 24 seconds: choose a clothing product video length | AI Clothes Video`
English Meta Description：`Choose clothing product video length by the number of buyer questions you need to answer. Compare the shot structure, source-image pressure, and best use for 8-, 16-, and 24-second videos.`
中文 SEO Title：`服装商品视频做 8 秒、16 秒还是 24 秒？｜AI Clothes Video`
中文 Meta Description：`按需要回答的购买问题选择服装商品视频时长，对比 8、16、24 秒的镜头结构、素材压力和适用场景。`
...
- [ ] 3 个目标 URL 均可访问，并返回独立页面内容。
- [ ] `/zh/guides` 索引页能发现全部文章。
- [ ] 三图专题页向下链接 3 篇文章。
