...
本产品是一款面向跨境/独立站中小服装卖家的自助式商品短视频生成工具。用户上传服装产品图、多角度图、细节图或场景图后，系统根据素材完整度和 Style Preset 风格预设推荐安全镜头模板，用户可一键采用推荐方案或在高级调整中二次选择模板，确认分镜后生成 8/16/24/32 秒宣传短视频；付费用户还可在服务端开关开放时使用 40 秒 Beta。

MVP 的目标不是做“全自动广告大片工厂”，而是验证一个可付费、可控、可复现的最小生成闭环：

- 用户是否愿意上传真实 SKU 素材。
- 系统是否能稳定生成可下载的短视频。
- 用户是否愿意为 8/16/24/32 秒成片和 40 秒付费 Beta 购买点数。
...
- 8/16/24/32 秒公开固定规格，以及受 `VIDEO_DURATION_40_ENABLED` 控制的 40 秒付费 Beta。
- 多个 8 秒片段拼接为一个完整视频。
- 生成后抽帧质检。
...
| 32 秒长版 | 4 个 8 秒片段 | 更完整的付费商品叙事，沿用普通分段生成和质检 |
| 40 秒付费 Beta | 5 个 8 秒片段 | 长版商品宣传，按片段与转场强化质检 |

32 秒消耗 250 点，不开放免费试用，使用 4 个普通有序镜头，不继承 40 秒五槽位组合限制。40 秒消耗 310 点，不开放免费试用。五个镜头槽位至少包含 3 种模板，同一模板最多出现 2 次、不得相邻重复，中高/高风险旋转或转身模板最多出现 1 次。
...
公开 MVP 工作台采用“三张同款有效素材”上传协议。用户必须先选择一种三图协议，并为三个明确位置分别上传同一 SKU 的有效图片；系统仍可在后续高级流程中保存补充素材，但不能把“有一张正面图”作为公开生成入口的完整条件。

### 6.1 公开流程必传

- 按所选协议上传三张同款有效素材。默认商品展示协议为正面主图 + 背面图 + 细节图。
...
- 商品展示：正面主图 + 背面图 + 细节图。
- 商品旋转付费 Beta：同款无真人商品正面 + 侧面 + 背面。
- 真人模特转身付费 Beta：同一模特穿同一件服装的正面 + 侧面 + 背面，并满足授权与任务内一致性要求。
- 其他左/右侧、细节或场景图可作为补充素材，但不替代三图协议中的必需位置。
- 场景图或氛围参考图。
...
普通用户默认通过 Style Preset 选择视频风格/用途；系统再根据 preset 偏好、素材完整度和模板规则自动推荐 1/2/3/4 个镜头模板，40 秒 Beta 则生成 5 个有序镜头槽位。模板选择仍然保留，但应作为高级调整能力展示。Style Preset 与模板的完整关系见 [Style Preset 风格预设设计](STYLE_PRESET_DESIGN.md)。
...
3. 选择视频规格：8/16/24/32 秒；开关开放时可选择 40 秒付费 Beta。
...
- 输出低分辨率。
- 带水印。
- 不开放高风险镜头、转身、背面展示、24/32 秒付费规格和 40 秒 Beta。
- 必须登录并邮箱验证。
...
- MVP 公开生成链路。
- 按 8 秒片段生成视频。
- 支持 8/16/24/32 秒最终成片和受开关控制的 40 秒付费 Beta，本质是 1/2/3/4/5 个独立 8 秒片段生成后拼接。
...
8 秒沿用普通抽帧计划：Standard 固定 5 帧，Strict 固定 6 帧。16/24/32 秒保留这组均匀帧，并额外抽取每个 8 秒拼接边界；Lite 保持 3 帧且不增加边界帧。40 秒 Standard 固定抽取 24 帧，Strict 固定抽取 34 帧，按 5 个片段批次加 1 个转场批次调用视觉模型。仅当一个片段被精确定位为失败且转场通过时，自动重试该片段一次；重试耗尽、多段失败、转场失败或供应商异常才进入失败释放点数流程。

Cloud Run 在上传最终视频前必须执行技术门禁：按任务 `aspect_ratio` 校验实际画幅，按生成档位校验最低短边（540/720/1080），并对 Standard/Strict 多段成片拒绝持续至少 1 秒的冻结。技术门禁失败按 stitch 失败回调，坏视频不得上传或进入交付链。
- 用户等待时间。
...
R2 写入仍使用 S3-compatible 鉴权和 signed upload URL；原图读取、任务二次预览、视觉分析与视频生成输入统一使用 `CLOUDFLARE_R2_PUBLIC_BASE_URL` 配置的 HTTPS 公共自定义域，不再生成短期 signed read URL。

公共 URL 本身是可转发、长期有效的访问凭证：应用只向任务所有者返回原图 URL，但任何获得 URL 的人都可以在对象删除前访问该文件。对象 key 必须保持不可枚举，数据库仍只保存 R2 key，不保存完整公共 URL。
...
- 删除动作写审计日志。
- 用户点击删除时不直接同步删除 R2。
- 公共域可能存在 CDN 缓存；删除完成时间必须包含缓存失效窗口。
