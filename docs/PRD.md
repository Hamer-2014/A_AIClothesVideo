...
本产品是一款面向跨境/独立站中小服装卖家的自助式商品短视频生成工具。用户上传服装产品图、多角度图、细节图或场景图后，系统根据素材完整度推荐安全镜头模板，用户二次选择模板并确认分镜，系统生成 8/16/24 秒宣传短视频。

MVP 的目标不是做“全自动广告大片工厂”，而是验证一个可付费、可控、可复现的最小生成闭环：

- 用户是否愿意上传真实 SKU 素材。
- 系统是否能稳定生成可下载的短视频。
- 用户是否愿意为 8/16/24 秒成片购买点数。
...
- 用户自己选择模板，系统给出推荐和禁用原因。
- DeepSeek 生成结构化分镜和提示词草稿。
- EvoLink Veo 3.1 Pro Beta 生成视频片段。
- 8/16/24 秒固定规格。
- 多个 8 秒片段拼接为一个完整视频。
- 生成后抽帧质检。
...
- Creem、DeepSeek、视觉模型、EvoLink 视频模型在开发早期即真实接入，不做 mock 成功链路。
...
4. 有真人时，转身和大幅肢体动作属于高风险镜头，MVP 默认不公开。
5. DeepSeek 只能引用已启用镜头模板 ID，不能创造新镜头模板。
6. 用户看到的是完整视频任务；后台可以看到每个 8 秒片段。
7. 模型调用、状态变化、点数变化必须可追踪、可复现、可审计。
...
- 免费试用：720p，带水印。
- 付费默认：1080p，无水印。
- 4K：不进入 MVP 公开售卖，仅后台/内测。

MVP 默认不生成音频。用户可在 TikTok/Reels 等平台自行添加音乐。
...
模板库是产品规则和运营资产，不是前端写死的按钮列表。MVP 只开放低风险和少量中风险模板，高风险模板后台默认关闭或仅内测。
...
- 推荐模板：素材满足要求、风险低、适合当前服装。
- 可选模板：素材满足要求，但风险稍高或效果不一定最佳。
...
- 输出 720p。
- 带水印。
- 不开放高风险镜头、转身、背面展示、24 秒增强版。
- 必须登录并邮箱验证。
- 需要防滥用：user ID、email、OAuth provider ID、IP、设备指纹。

### 11.3 点数消耗建议

| 规格 | 输出 | 消耗点数 |
|---|---|---:|
| 8 秒基础版 | 1080p，无水印 | 70 点 |
| 16 秒标准版 | 1080p，无水印 | 130 点 |
| 24 秒增强版 | 1080p，无水印 | 190 点 |
...
- 视觉识别/质检模型真实接入。
- EvoLink `veo3.1-fast-beta` 真实接入。
- APIMart `pixverse-v6` 可真实接入，但仅管理员内测。
- 不做“假成功”的 mock 生成链路。
...
- MVP 公开生成链路。
- 按 8 秒片段生成视频。
- 支持 8/16/24 秒最终成片，本质是 1/2/3 个片段生成后拼接。
...
- 16/24 秒付费任务默认 `standard`。
- 有真人素材、背面展示、正背切换、中高风险模板强制 `strict`。
...
- EvoLink `veo3.1-fast-beta` 实测成功率、耗时、成本。
