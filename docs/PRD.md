...
8/16/24/32 秒沿用普通抽帧计划：Standard 固定 5 帧，Strict 固定 6 帧。40 秒 Standard 固定抽取 24 帧，Strict 固定抽取 34 帧，按 5 个片段批次加 1 个转场批次调用视觉模型。仅当一个片段被精确定位为失败且转场通过时，自动重试该片段一次；重试耗尽、多段失败、转场失败或供应商异常才进入失败释放点数流程。
- 用户等待时间。
