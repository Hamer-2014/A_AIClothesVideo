...
- 键盘顺序遵循页面阅读顺序：协议 -> 授权 -> 三槽位 -> 规格与风格 -> 主 CTA -> 可选补充信息 -> 高级内容。
...
- “真实生成样片”只指由当前项目生成链路实际产出并通过复核的视频，不得用静态图、动效 mock 或第三方未知样片冒充 Preset 输出。
- 首页与案例页允许使用可追溯的合成商品输入，但必须同时满足：明确标注“合成演示、非客户案例”；记录生成模型、最终 prompt、尺寸和 SHA-256；不得写成客户案例或暗示已有对应成片。
- 当前 demo 资产位于 `public/demo/`。WebP 单图最长边不超过 1200px，视频使用 H.264 MP4 并启用 faststart；合成素材的审计信息与对应 SKU 存放在同一目录。
