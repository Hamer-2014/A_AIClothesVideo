...
- [ ] 生成或保存缩略图。
- [ ] 实现用户文件访问权限校验。
- [ ] 上传使用 signed URL；原图预览和模型访问强制使用 `CLOUDFLARE_R2_PUBLIC_BASE_URL`。
- [ ] 实现删除标记 `deleted_at`。
- [ ] 预留异步清理任务。

**验收：**

- [ ] 应用只向素材所有者返回源图公共 URL，不泄露其他用户的对象 key。
- [ ] 公共自定义域必须为 HTTPS，且对象 key 不可枚举。
- [ ] 缺少或误配公共域时 readiness 失败，源图读取不回退 signed URL。
- [ ] 删除不会在用户请求中同步阻塞。
...
- [x] 多段失败、转场失败和 schema 异常沿用失败释放流程；Post-QA 网络/超时/429/5xx 按当前 reserve 最多退避重试两次，耗尽后才失败释放。
- [x] Post-QA capture/release 与管理员 release 按 `reserved_ledger_id` 独立幂等；管理员重开已结算付费任务时原子重新 reserve 并更新任务关联。
- [x] Post-QA 结算账本已写但终态更新中断时由 cron 复用持久化 QA 结果恢复；未找到当前 reserve 对应结算账本时不自动 release。
