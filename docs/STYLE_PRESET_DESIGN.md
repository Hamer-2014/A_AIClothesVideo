...
- 普通用户主要选择风格/用途，而不是理解当前 17 个镜头模板。
- 系统根据素材完整度自动推荐模板组合。
- 模板选择作为“高级调整”折叠展示。
...
`product_quarter_rotation` 与 `product_half_rotation` 是付费 Beta、Advanced-only 模板，`autoSelectAllowed=false`。即使商品 front/side/back 和同款一致性全部通过，它们也只进入“可选模板”，不能进入任何 Preset 的自动槽位。两个模板只处理无真人商品图，不生成真人/虚拟模特，也不做虚拟穿衣。
...
只有商品图时，Preset 不得隐式生成真人或虚拟模特。虚拟穿衣是未来独立上游模块；其输出被明确识别为 `human_model` 并通过任务内一致性校验后，才可复用模特动作模板。
...
`social_lifestyle` 风险最高。没有场景图时，不允许生成强场景；没有模特正面图时，不允许启用任何模特动作模板；只有单张模特正面图时仅允许 `model_front_pose`，不能启用转身；试用模式下应优先降级到低风险模板。
...
6. 过滤 `autoSelectAllowed=false` 的高级模板；它们仍保留在用户可选列表。
7. 根据时长自动选择模板数量：
   - 8 秒：1 个模板。
   - 16 秒：2 个模板。
   - 24 秒：3 个模板。
...
- 免费试用只使用低风险模板。
- 商品旋转 Beta 即使素材满足也不能被 Preset 自动选中；必须由付费用户在高级调整中明确选择，并强制 Strict QA。
