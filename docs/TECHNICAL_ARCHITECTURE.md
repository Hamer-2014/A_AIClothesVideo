...
| 视频生成 | APIMart `pixverse-v6` |
| 备用视频模型 | EvoLink `veo3.1-fast-beta` |
...
- 视觉识别/质检模型。
- APIMart `pixverse-v6`。
- EvoLink `veo3.1-fast-beta`，备用/对照路线。
...
3. 提交 APIMart `pixverse-v6` 异步生成任务。
...
| `video_generation` | APIMart `pixverse-v6` | MVP 主视频模型 |
| `experimental_video` | EvoLink `veo3.1-fast-beta` | 备用/对照 |
...
- APIMart `pixverse-v6` 的实测时长、价格、失败率。
