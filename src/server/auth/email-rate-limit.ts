...
interface AuthEmailRateLimitContext {
  ...
  rateLimitError: AuthEmailRateLimitError | null;
}
...
export function recordAuthEmailRateLimitError(error: AuthEmailRateLimitError) {
  ...
  if (context) context.rateLimitError = error;
}

export function recordAuthEmailDeliveryError(error: unknown) {
  const context = authEmailRateLimitContext.getStore();
  if (context) context.deliveryError = error;
}

export async function runWithAuthEmailDeliveryCapture<T>(
  operation: () => Promise<T>,
) {
  ...
  try {
    ...
    return {
      ...
      operationError: null,
      operationFailed: false as const,
      rateLimitError: context.rateLimitError,
      result,
    };
