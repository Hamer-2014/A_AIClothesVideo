...
describe("virtual try-on runtime tick", () => {
  it("persists a front task and does not submit it again", async () => {
    ...
    const store: RuntimeStore = { acquire: async () => job, saveAsset: async (_id, asset) => { job.assets[0] = asset; }, setStatus: async (_id, status) => { job.status = status; }, releaseLease: async () => undefined };
  ...
  it("persists a retry delay after the first retryable submit failure", async () => {
    const job: RuntimeJob = { id: "job", packId: "pack", userId: "user", mode: "front_only", status: "queued", creditCost: 2, lockedUntil: null, assets: [{ view: "front", providerTaskId: null, providerStatus: "pending", attemptCount: 0, r2Key: null }] };
    const saved: RuntimeJob["assets"][number][] = []; let released = 0;
    const store: RuntimeStore = { acquire: async () => job, saveAsset: async (_id, asset) => { saved.push(asset); }, setStatus: async (_id, status) => { job.status = status; }, releaseLease: async () => { released++; } };
    const credits = createInMemoryCreditLedgerStore(); await grantTrialCredits({ store: credits, userId: "user", amount: 2, reason: "test", idempotencyKey: "grant" });
    ...
    expect(result.action).toBe("retry"); expect(saved[0]?.attemptCount).toBe(1); expect(saved[0]?.nextRetryAt).toEqual(new Date(30_000)); expect(released).toBe(1);
  });
});
