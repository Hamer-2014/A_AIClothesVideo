...
describe("virtual try-on runtime tick", () => {
  ...
  it("returns lost_lease without resolving QA when renewal fails", async () => {
    const { store } = qaReadyStore("qa_queued");
    const credits = await reservedCredits();
    const visionProvider = vi.fn();
    const resolveQa = vi.fn(store.resolveQa);
    store.resolveQa = resolveQa;
    store.renewLease = vi.fn().mockResolvedValue(false);

    const result = await runVirtualTryOnTick({ workerId: "worker", store, credits, submit: async () => "unexpected", poll: async () => ({ status: "succeeded", outputUrl: null }), qaDeps: { signer: async (key) => "https://signed.example/" + key, visionProvider, qaStore: createInMemoryVirtualTryOnQaStore(), providerLogStore: createInMemoryProviderCallLogStore() } });

    expect(result.action).toBe("lost_lease");
    expect(visionProvider).not.toHaveBeenCalled();
    expect(resolveQa).not.toHaveBeenCalled();
  });

  it("persists transferred MIME and byte size for later video materialization", async () => {
    const job: RuntimeJob = { id: "job", packId: "pack", userId: "user", mode: "front_only", status: "generating", creditCost: 2, lockedUntil: null, sourceKeys: { front: "source" }, modelKeys: { front: "models/front", side: "models/side", back: "models/back" }, assets: [{ view: "front", providerTaskId: "task", providerStatus: "succeeded", attemptCount: 1, r2Key: null }] };
    let saved: RuntimeJob["assets"][number] | undefined;
    const store = { acquire: async () => job, saveAsset: async (_id: string, _worker: string, asset: RuntimeJob["assets"][number]) => { saved = { ...asset }; return true; }, transitionToGenerating: async () => true, transitionAssetsReadyToQaQueued: async () => true, resolveQa: async () => true, finalizeCapturedPack: async () => true, scheduleCapturePersistenceRetry: async () => "retry" as const, transitionCaptureToRefund: async () => true, transitionToRecoveringRelease: async () => true, releaseLease: async () => true, scheduleRetry: async () => true } as unknown as RuntimeStore;

    const result = await runVirtualTryOnTick({
      workerId: "worker",
      store,
      credits: createInMemoryCreditLedgerStore(),
      submit: async () => { throw new Error("must_not_submit"); },
      poll: async () => ({ status: "succeeded", outputUrl: "https://upload.apimart.ai/output.webp" }),
      transfer: async () => ({ key: "virtual-try-on/job/packs/pack/front.png", contentType: "image/webp", fileSize: 4096 }),
    });

    expect(result.action).toBe("transfer");
    expect(saved).toMatchObject({ r2Key: "virtual-try-on/job/packs/pack/front.png", mimeType: "image/webp", fileSize: 4096 });
  });
});
