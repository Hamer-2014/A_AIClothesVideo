...
describe("APIMart image provider", () => {
  it("parses the official submit envelope and sends the fixed 9:16 2k full-body protocol", async () => {
    ...
    expect(JSON.parse(String(fetchSpy.mock.calls[0]?.[1]?.body))).toEqual({ model: "gpt-image-2", n: 1, size: "9:16", resolution: "2k", prompt: "front", image_urls: ["model", "front", "back", "detail"] });
  ...
  it.each([
    ["pending", "queued"],
    ["cancelled", "failed"],
  ] as const)("maps the official %s task status to %s", async (providerStatus, expectedStatus) => {
    await expect(pollAPIMartImageTask("task-1", {
      fetch: async () => new Response(JSON.stringify({ data: { status: providerStatus } }), { status: 200 }),
      apiKey: "key",
    })).resolves.toMatchObject({ status: expectedStatus, outputUrl: null });
  });
