...
import type { VirtualTryOnMode, VirtualTryOnPublicConfig } from "@/server/virtual-tryon/config";
...
export function VirtualTryOnCreateForm({ language, publicConfig = { available: true, frontOnlyCreditCost: null, threeViewCreditCost: null } }: { language: SiteLocale; publicConfig?: VirtualTryOnPublicConfig }) {
  ...
  const selectedCreditCost = mode === "front_only" ? publicConfig.frontOnlyCreditCost : publicConfig.threeViewCreditCost;
  const unavailableMessage = workspaceText(language, "The virtual try-on service is temporarily unavailable. Try again shortly.", "虚拟试穿服务暂不可用，请稍后重试。");
  const canSubmit = publicConfig.available && rightsAccepted && allRequiredUploaded && !uploading && state !== "submitting" && state !== "submitted";
  ...
  function changeMode(nextMode: VirtualTryOnMode) {
    if (nextMode === mode || state === "submitting" || !publicConfig.available) return;
    setMode(nextMode);
  ...
  return (
    <form className="border border-[var(--line)] bg-[var(--surface-raised)]" data-state={state} onSubmit={submit}>
      ...
      <div className="space-y-5 px-4 py-5 sm:px-6">
        <UploadPanel
          assets={assets}
          disabled={state === "submitting" || state === "submitted" || !publicConfig.available}
          language={language}
        ...
        <div className="max-w-md">
          ...
          <input className="mt-2 h-10 w-full rounded-[var(--radius-md)] border border-[var(--line)] bg-white px-3 text-sm outline-none placeholder:text-[var(--muted)] focus:border-[var(--action)] focus:ring-2 focus:ring-[var(--focus)] disabled:bg-[var(--surface-subtle)]" disabled={state === "submitting" || !publicConfig.available} id="virtual-try-on-sku" maxLength={80} onChange={(event) => setSkuName(event.target.value)} placeholder={workspaceText(language, "e.g. Linen Dress / SKU-1024", "例如亚麻连衣裙 / SKU-1024")} value={skuName} />
        </div>
        ...
        <div className="flex flex-col gap-3 border-t border-[var(--line)] pt-4 sm:flex-row sm:items-center sm:justify-between">
          <div aria-live="polite" className="min-h-5 text-sm text-[var(--muted)]">
            {!publicConfig.available ? <p role="alert">{unavailableMessage}</p> : statusMessage ? <p role={message ? "alert" : undefined}>{statusMessage}</p> : null}
          </div>
          <button className="inline-flex h-11 w-full shrink-0 items-center justify-center gap-2 rounded-[var(--radius-md)] bg-[var(--action)] px-4 text-sm font-semibold text-white transition hover:bg-[var(--action-hover)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus)] disabled:cursor-not-allowed disabled:bg-[var(--line-strong)] sm:w-auto" disabled={!canSubmit} type="submit">
            ...
            {state === "submitting" ? workspaceText(language, "Submitting appearance pack", "正在提交定妆图") : state === "submitted" ? <><Check aria-hidden="true" size={16} />{workspaceText(language, "Opening task", "正在打开任务")}</> : `${workspaceText(language, "Create appearance pack", "创建定妆图")}${selectedCreditCost !== null ? ` · ${selectedCreditCost} ${workspaceText(language, "credits", "点数")}` : ""}`}
          </button>
        </div>
      </div>
    </form>
